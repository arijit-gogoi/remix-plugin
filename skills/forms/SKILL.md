---
name: remix-forms
description: HTML form parsing and streaming file uploads in Remix v3 — the `formData()` middleware (`remix/form-data-middleware`) and one-off `parseFormData(request, opts)` (`remix/form-data-parser`) for endpoints that don't go through the full middleware stack. Load whenever the user is **wiring** an HTML form (`<form enctype="multipart/form-data">`), **handling** a file upload, **customising** an `uploadHandler` (where bytes go — disk / memory / S3 / virus-scan tee / reject by content-type), **tightening** per-route limits (`maxFileSize`, `maxFiles`, `maxFieldSize`, `maxFields`), **streaming** a large upload (500MB CSV) without buffering, **hitting** `MaxFileSizeExceededError` / `MaxFilesExceededError`, reading `get(FormData)` inside a controller, or using `_method=PUT` with `methodOverride()`. About to install **`multer`**, **`busboy`**, **`formidable`**, **`express-fileupload`**, **`koa-multer`**, **`@fastify/multipart`**? Remix v3 ships all of it — read this skill first. For **where the file lives after upload** (filesystem / S3 / memory backends), see /remix:file-storage; for **validating** the parsed fields against a schema, see /remix:validation.
---
