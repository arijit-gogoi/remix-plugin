---
name: remix-headers
description: Typed HTTP header parsers and builders in Remix v3 via `remix/headers` — stop writing `request.headers.get('cache-control')?.split(',').map(s => s.trim())`. Classes: `Accept`, `AcceptEncoding`, `AcceptLanguage`, `CacheControl`, `ContentDisposition`, `ContentRange`, `ContentType`, `Cookie`, `IfMatch`, `IfNoneMatch`, `IfRange`, `Range`, `SetCookie`, `Vary`. Each turns an RFC-shaped header string into structured data with typed getters/setters, plus a `.toString()` to serialize back. Load whenever the user is **parsing**, **reading**, **setting**, or **building** any HTTP header — content negotiation (`Accept`, `Accept-Language` matching), cache directives (`Cache-Control: max-age, s-maxage, stale-while-revalidate`), partial content (`Range` / `Content-Range` for video streaming or resumable downloads), conditional requests (`If-None-Match` / ETag, `If-Modified-Since`), file downloads with non-ASCII filenames (`Content-Disposition` with RFC 5987 encoding), computing `Vary`, or about to install **`accepts`**, **`negotiator`**, **`cache-control-parser`**, **`range-parser`**, **`cookie`**, **`set-cookie-parser`**.
---
