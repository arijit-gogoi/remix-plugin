...
description: Schema migrations in Remix v3 — `createMigration({ up, down })`, the timestamped `db/migrations/<YYYYMMDDHHMMSS>_<name>.ts` filename convention, and the migration runner (`up`, `down`, `status`, `latest`, `current`). Use the `schema` DDL surface inside `up`/`down`: `createTable`, `alterTable` (add/drop/rename column), `dropTable`, `createIndex`, `addForeignKey`, raw `schema.exec(sql)` for backfills. Load whenever the user wants to **create**, **apply**, or **roll back** a migration; rename a column on a large table; add a NOT NULL column with a default; design a zero-downtime schema change (expand/migrate/contract); add a foreign key with ON DELETE CASCADE; debug "checksum mismatch" or "migration locked" errors; handle parallel-deploy races via advisory lock; or compare to **Drizzle Kit**, **Prisma Migrate**, **Knex migrations**, **Flyway**, **Liquibase**, **TypeORM migrations**, or **db-migrate**. For query execution (find/create/update/delete on tables), see /remix:database — this skill is only for schema evolution.
---
