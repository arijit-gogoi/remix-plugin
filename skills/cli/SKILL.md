---
name: remix-cli
description: The **official** `remix` CLI that ships with the framework — six commands: `remix new <name>` (bootstrap a fresh v3 project, `--app-name`), `remix doctor [--fix]` (health-check against v3 conventions, auto-repair low-risk drift), `remix routes [--tree|--table|--json]` (inspect the typed route tree, machine-readable output for docs), `remix test [glob]` (run tests via the built-in runner), `remix version`, `remix completion bash|zsh` (shell completions). Programmatic: `runRemix([...args])` from `remix/cli`. Load whenever the user is **bootstrapping a new project** (`npx remix@next new`), running `remix doctor` and wondering about exit codes / safety of `--fix`, **dumping the route tree** for a PR diff or docs, generating shell completions, comparing the bootstrap to **`create-next-app`** / **`npm create vite`** / **`npm create vue`** / **Rails `bin/rails new`** / **Laravel `composer create-project`**, or asking what the scaffolded project layout looks like. For **incremental** scaffolders that the official CLI doesn't ship (add one route, add one resource, add one migration, insert one middleware), see /remix:scaffolders — that's a different skill.
---
