...
description: Reference card for the smaller utility packages Remix v3 ships — mime (content-type detection), lazy-file (streaming file abstraction), fs (Node fs wrappers returning LazyFile), tar-parser (parse tar archives), terminal (ANSI styling), node-serve (uWebSockets.js high-perf server), fetch-proxy (reverse-proxy helper), plus pointers to the larger utilities that have their own sub-skills (templating, headers, assets). Load when looking for a built-in helper for content-type, file streaming, terminal coloring, tar parsing, alternative server, or proxying.
---
