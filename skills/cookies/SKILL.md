---
name: remix-cookies
description: Cookies in Remix v3 via `remix/cookie` — the low-level building block sessions, auth, and CSRF build on. Load whenever the user is **setting**, **reading**, **clearing**, or **rotating** a cookie: signed cookies with `createCookie(name, { secrets: [...] })` and the rotation playbook (`['NEW', 'OLD']`), parsing the `Cookie` request header, building `Set-Cookie` response headers; cookie attributes — `httpOnly`, `secure` (and the "cookie not sent in dev over HTTP" trap), `sameSite: 'Lax' | 'Strict' | 'None'` (including iframes / cross-site embeds), `maxAge` vs `expires` (and how to **clear** a cookie), `path`, `domain` (subdomain sharing: `app.example.com` + `admin.example.com`), `priority`, **`partitioned` / CHIPS** for third-party contexts; building **GDPR consent banners**, dark-mode flags, locale prefs, A/B buckets, "remember me", feature flags. Also load when about to install **`cookie`** or **`cookie-signature`** — Remix ships both. For signed sessions on top of cookies see /remix:sessions; for the typed `Cookie` / `SetCookie` header **parsers** (not the storage primitive) see /remix:headers.
---
